...
func NewSocketEnricher() (*SocketEnricher, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("error opening udp_sendmsg ipv4 kprobe: %w", err)
	}
	se.links = append(se.links, l)

	l, err = link.Kprobe("udpv6_sendmsg", se.objs.IgUdp6Sendmsg, nil)
	if err != nil {
		return nil, fmt.Errorf("error opening udpv6_sendmsg ipv4 kprobe: %w", err)
	}
	se.links = append(se.links, l)
...
func (se *SocketEnricher) PlugExtension(target *ebpf.Program, netns uint64) (io.Closer, error) {
	...
	for funcName := range spec.Programs {
