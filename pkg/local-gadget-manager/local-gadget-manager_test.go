	...
	for i := 0; i < 4; i++ {
		...
		if i%2 == 0 {
			testutils.RunDockerContainer(context.Background(), t,
				testutils.WithName("test-local-gadget-failed-container"),
				testutils.WithImage("docker.io/library/busybox"),
				testutils.WithCommand("/none"),
...
func TestDNS(t *testing.T) {
	...
	testutils.RunDockerContainer(context.Background(), t,
		...
		testutils.WithCommand("dig @"+nameserver+" magic-1-2-3-4.nip.io"),
	)
...
func TestNetworkGraph(t *testing.T) {
	...
	testutils.RunDockerContainer(context.Background(), t,
		...
		testutils.WithCommand("nginx && curl 127.0.0.1"),
	)
