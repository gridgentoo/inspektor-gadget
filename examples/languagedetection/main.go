...
import (
	...
	langaugesTracer "github.com/inspektor-gadget/inspektor-gadget/pkg/gadgets/profile/languages/tracer"
...
func main() {
	...
	// Create the tracer
	config := &langaugesTracer.Config{MountnsMap: mountnsmap}
	tracer, err := langaugesTracer.NewTracer(config, containerCollection)
	if err != nil {
		fmt.Printf("error creating tracer: %s\n", err)
		return
	}

	fmt.Print("hit ctrcl to finish...")
	...
	ret, err := tracer.Stop()
	if err != nil {
		fmt.Printf("error stopping tracer: %s\n", err)
		return
	}

	fmt.Printf("\nnamespace, pod, container, language, exections\n")

	for key, val := range ret.Languages {
